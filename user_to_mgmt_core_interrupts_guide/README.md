...
Look at how these were configured in the modified `crtl_0.S` in the firmware folder. The
stack saving before calling `irq_handler` in `crtl_0.S` may not actually be necessary though.

I assume this doesn't have to be setup in the `crtl_0.S` and can be done later and in a more comfortable place (not inside the firmware folder)
