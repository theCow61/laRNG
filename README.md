...
to an FPGA, so a different latch based entropy source, the auxiliary entropy source, was added which is suppose to be FPGA friendly where the presets and resets of two latches are intertwined. The FPGA friendly entropy source was helpful for prototyping the design as a whole.
...
| 1 | AuxEnable | RW | 0 | 1: Auxiliary entropy source is enabled and combined with with primary entropy source. <br>0: Random generated bits come primarily from the primary entropy source. |
